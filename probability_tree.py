...
class SumNode:
    ...
    def record_participant(self, participant: bj.Participant) -> None:
        """Record the actions of the participant to calculate probabilities of any future moves or if they should be
        a move at all
...
class ProbabilityTree:
    ...
    ** where the root is the sum that we want to visualize all possible card sums / outcomes **
    ...
    - self._subtrees is empty iff the root.current_total >= target value for cards (ex: if target is 21, no
...
def generate_example_for_graphing() -> ProbabilityTree:
    """ Generates an example prability tree for graphing
    """
    root = ProbabilityTree(19)

    black_jack_intialized = bj.BlackJack()
    thedeck = black_jack_intialized.deck.copy_deck_and_draw_specific_card('9')
    thedeck = thedeck.copy_deck_and_draw_specific_card('10')
    thedeck = thedeck.copy_deck_and_draw_specific_card('10')
    thedeck = thedeck.copy_deck_and_draw_specific_card('10')

    return root.generate_tree(thedeck, 21)


def run_example_tree() -> ProbabilityTree:
    """Return a ProbabilityTree given the methods from
    black_jack_game for intializing cards for the dealer and player with target==21"""
...
if __name__ == '__main__':
    ...
    python_ta.check_all(config={
        'extra-imports': ['annotations', 'Optional', 'black_jack_game', 'networkx', 'matplotlib.pyplot'],
        'allowed-io': [],
        'max-line-length': 120
    })
